...
<style>
    ...
    .products-table thead { background: #f8fafc; }
    ...
    .products-table th:first-child { text-align: left; }
    ...
    .product-name { font-weight: 500; color: var(--text-dark); }
    .product-code { 
        font-size: 0.75rem; 
        background: #dcfce7; 
        color: #16a34a; 
    ...
    .total-display { font-weight: 600; color: var(--success); }
    ...
    .action-btn-save { background: var(--success-light); color: var(--success); }
    .action-btn-save:hover { background: var(--success); color: white; }
    ...
    .action-btn-add:hover { background: var(--accent-hover); }
    ...
    .add-product-row td { background: #f8fafc; border-top: 1px solid #e2e8f0; }
    ...
    @media (max-width: 768px) {
        ...
        .mobile-cards { display: block; padding-bottom: 100px; }
        ...
        .mobile-product-name { font-weight: 600; font-size: 0.95rem; color: var(--text-dark); flex: 1; }
        .mobile-product-actions { display: flex; gap: 6px; }
        .mobile-product-actions .action-btn { width: 32px; height: 32px; }

        .mobile-product-details { display: flex; justify-content: space-between; gap: 1rem; }

        .mobile-detail-item { text-align: center; flex: 1; }
        .mobile-detail-label { font-size: 0.65rem; text-transform: uppercase; color: var(--text-light); margin-bottom: 2px; }
        .mobile-detail-value { font-size: 0.85rem; font-weight: 600; color: var(--text-dark); }
        .mobile-detail-value.total { color: var(--success); }

        .mobile-edit-form { padding: 1rem; }
        .mobile-form-group { margin-bottom: 1rem; }
        .mobile-form-group label { display: block; font-size: 0.7rem; font-weight: 600; text-transform: uppercase; color: var(--text-light); margin-bottom: 6px; }
        .mobile-form-group select, .mobile-form-group input { width: 100%; padding: 12px; border: 1px solid #e2e8f0; border-radius: 10px; font-size: 1rem; background: #f8fafc; }

        .mobile-form-actions { display: flex; gap: 8px; margin-top: 1rem; }
        .mobile-form-actions .action-btn { flex: 1; height: 44px; font-size: 0.85rem; gap: 6px; }
        ...
        .mobile-summary-row { display: flex; justify-content: space-between; padding: 0.75rem 1rem; border-bottom: 1px solid #f1f5f9; }
        .mobile-summary-row:last-child { border-bottom: none; }
        .mobile-summary-row.grand-total { background: var(--primary); color: white; }
        .mobile-summary-label { font-size: 0.85rem; color: var(--text-light); }
        .mobile-summary-value { font-size: 0.9rem; font-weight: 600; }
        .mobile-summary-row.grand-total .mobile-summary-label, .mobile-summary-row.grand-total .mobile-summary-value { color: white; }
        ...
        .mobile-sticky-footer .btn-update, .mobile-sticky-footer .btn-complete {
            flex: 1;
            justify-content: center;
            padding: 14px 16px;
            font-size: 0.85rem;
        ...
        <tbody>
            ...
            <tr>
                <td>
                    ...
                    <span class="product-code">{{ $invoiceProduct['product_code'] }}</span>
                    @else
                ...
                <td class="text-center">
                    @if($invoiceProduct['is_saved'])
                    <span class="price-display">{{ format_currency($invoiceProduct['product_price'] / 1.16) }}</span>
                    @endif
                </td>
                <td class="text-center">
                    <span class="total-display">{{ format_currency(($invoiceProduct['product_price'] * $invoiceProduct['quantity']) / 1.16) }}</span>
                </td>
                <td class="text-center">
            ...
            <tr class="totals-row">
                <th colspan="5">{{ __('Subtotal (excl. VAT)') }}</th>
                <td class="text-center total-display">{{ format_currency($subtotal / 1.16) }}</td>
            </tr>

            <tr class="totals-row">
                <th colspan="5">{{ __('Discount') }}</th>
                <td>
                    <div style="display: flex; align-items: center; justify-content: center; gap: 6px;">
                        <input wire:model.blur="discountPercentage" type="number" class="summary-input" min="0" max="100" step="0.01">
                        <span>%</span>
                        <span style="color: var(--danger); font-size: 0.85rem; margin-left: 8px;">(-{{ format_currency($discountAmount) }})</span>
                    </div>
                </td>
            </tr>

            <tr class="totals-row">
                <th colspan="5">{{ __('VAT (16% Incl.)') }}</th>
                <td class="text-center" style="font-weight: 600;">{{ format_currency($vatAmount) }}</td>
            </tr>

            <tr class="totals-row">
                <th colspan="5">{{ __('Shipping') }}</th>
                <td>
                    <div style="display: flex; align-items: center; justify-content: center; gap: 6px;">
                        <input wire:model.blur="shippingAmount" type="number" class="summary-input" min="0" step="0.01">
                    </div>
                </td>
            </tr>

            <tr class="totals-row grand-total">
                <th colspan="5">{{ __('Grand Total') }}</th>
                <td class="text-center" style="font-size: 1.1rem;">{{ format_currency($total) }}</td>
            </tr>
        </tbody>
    </table>
</div>
...
<div class="mobile-cards">
        ...
        <div class="mobile-product-card">
            <div class="mobile-product-header">
                ...
                <div class="mobile-product-actions">
                    <button type="button" wire:click="editProduct({{$index}})" class="action-btn action-btn-edit">
                        <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg>
                    </button>
            ...
            <div class="mobile-product-details">
                ...
                <div class="mobile-detail-item">
                    ...
                    <div class="mobile-detail-value">{{ format_currency($invoiceProduct['product_price'] / 1.16) }}</div>
                </div>
                <div class="mobile-detail-item">
                    <div class="mobile-detail-label">{{ __('Total') }}</div>
                    <div class="mobile-detail-value total">{{ format_currency(($invoiceProduct['product_price'] * $invoiceProduct['quantity']) / 1.16) }}</div>
                </div>
            </div>
        </div>
        @else
        <div class="mobile-product-card">
            <div class="mobile-edit-form">
                ...
                <div class="mobile-form-actions">
                    ...
                    @endif
                    <button type="button" wire:click="removeProduct({{$index}})" class="action-btn action-btn-delete">
                        <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"></polyline></svg>
                    </button>
                </div>
            </div>
        </div>
    ...
    <div class="mobile-summary">
        <div class="mobile-summary-row">
            ...
            <span class="mobile-summary-value">{{ format_currency($subtotal / 1.16) }}</span>
        </div>
        <div class="mobile-summary-row">
            <span class="mobile-summary-label">{{ __('Discount') }}</span>
            <span class="mobile-summary-value">{{ $discountPercentage }}% (-{{ format_currency($discountAmount) }})</span>
        </div>
        <div class="mobile-summary-row">
            <span class="mobile-summary-label">{{ __('VAT (16%)') }}</span>
            <span class="mobile-summary-value">{{ format_currency($vatAmount) }}</span>
        </div>
        <div class="mobile-summary-row">
            <span class="mobile-summary-label">{{ __('Shipping') }}</span>
            <span class="mobile-summary-value">{{ format_currency($shippingAmount) }}</span>
        </div>
        <div class="mobile-summary-row grand-total">
            <span class="mobile-summary-label">{{ __('Total') }}</span>
            <span class="mobile-summary-value" style="font-size: 1.1rem;">{{ format_currency($total) }}</span>
        </div>
    </div>
</div>
...
<div class="desktop-footer" style="padding: 1.5rem; background: #f8fafc; border-top: 1px solid #e2e8f0; display: flex; justify-content: flex-end; gap: 12px;">
    <button type="button" wire:click="updateQuotation" class="btn-update">
    ...
    <button type="button" wire:click="completeQuotation" wire:confirm="Are you sure you want to complete this quotation? Stock will be reduced." class="btn-complete">
...
<div class="mobile-sticky-footer">
    ...
    <button type="button" wire:click="completeQuotation" wire:confirm="Complete this quotation?" class="btn-complete">
        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="20 6 9 17 4 12"></polyline></svg>
        {{ __('Complete') }}
    </button>
</div>
